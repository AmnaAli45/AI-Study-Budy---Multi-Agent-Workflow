...
PASS_THRESHOLD_PERCENT = 60  # >= 60% correct -> pass
...
class QuizListSchema(BaseModel):
    questions: List[QuizSchema] = Field(
        description=f"Exactly {TOTAL_QUIZ_QUESTIONS} multiple choice questions."
    )
...
quiz_struc_model = model.with_structured_output(QuizListSchema, strict=True)
answer_struc_model = model.with_structured_output(AnswerCheckSchema, strict=True)

# ------------------------------------- Prompts -----------------------------------------
QUIZ_GENERATOR_PROMPT = f"""You are a quiz master inside an AI Study Buddy pipeline.

Given a study note on a topic, generate EXACTLY {TOTAL_QUIZ_QUESTIONS} multiple choice questions
to test the student on that topic.

Rules:
- Generate exactly {TOTAL_QUIZ_QUESTIONS} distinct questions (no duplicates, no repeated wording).
- Cover different sub-aspects of the topic (definitions, how it works, examples, edge cases) so the
  quiz feels well-rounded rather than repetitive.
...
- Difficulty should match the student's level:
  - beginner: straightforward concept checks
  - intermediate: application or "why" based questions
  - advanced: edge cases, tradeoffs, or internals based questions
- Order questions roughly from easier to harder.

Return structured output only — a list of {TOTAL_QUIZ_QUESTIONS} questions.
"""
...
RETRY_KEYWORDS = ["retry", "retake", "try again", "dubara", "dobara", "phir se", "restart quiz"]
...
def quiz_generator(state: State) -> dict:
    ...
    # Safety net: if the model returns fewer/more than expected, normalize the count.
    if len(questions) > TOTAL_QUIZ_QUESTIONS:
        questions = questions[:TOTAL_QUIZ_QUESTIONS]
...
def answer_checker(state: State) -> dict:
    ...
    quiz_list = state["quiz_questions"]
    quiz = quiz_list[idx]
    user_answer = state["messages"][-1].content
...
Student's answer: {user_answer}''')
    ...
    new_score = state["score"] + (1 if result.is_correct else 0)
    new_index = idx + 1
    is_last_question = new_index >= len(quiz_list)
    ...
    return {
        ...
        "last_answer_correct": result.is_correct,
        "last_feedback": result.feedback,
        "quiz_complete": is_last_question,
    }


def after_answer_router(state: State) -> Literal["evaluate_quiz", "__end__"]:
    """After each answer: if quiz just finished -> evaluate it, else stop and wait for next answer."""
...
# ------------------------------------------ Evaluate Quiz Node ---------------------------------------
def evaluate_quiz(state: State) -> dict:
    score = state["score"]
    total = state.get("total_questions") or len(state["quiz_questions"])
    ...
    if passed:
        summary = (
            ...
            f"Result: PASS\n\n"
            f"Great job! You've shown solid understanding of this topic."
        )
    else:
        summary = (
            f"Quiz Complete!\n\n"
            f"Score: {score}/{total} ({percentage}%)\n"
            f"Result: FAIL (need {PASS_THRESHOLD_PERCENT}% to pass)\n\n"
            f"No worries — review the notes above and reply 'retry' to attempt the quiz again."
        )

    return {
        "quiz_result": "pass" if passed else "fail",
        "quiz_complete": True,
...
#  --------------------------------------------- Entry Router ---------------------------------------
def entry_router(state: State) -> Literal["Understand_topic", "answer_checker", "quiz_generator"]:
    last_user_text = state["messages"][-1].content if state.get("messages") else ""

    # Case 1: quiz finished (pass or fail) and user wants to retry -> regenerate quiz
    if state.get("quiz_complete") and state.get("quiz_result") and _wants_retry(last_user_text):
        return "quiz_generator"

    # Case 2: quiz is in progress (questions exist, not yet complete) and user just answered
    if state.get("quiz_questions") and not state.get("quiz_complete") and state.get("messages"):
        return "answer_checker"

    # Case 3: fresh topic request
    return "Understand_topic"
...
graph.add_edge('search_agent', 'synthesize')
graph.add_edge('synthesize', 'quiz_generator')
graph.add_edge('quiz_generator', END)                       # first question goes out, wait for user
graph.add_conditional_edges('answer_checker', after_answer_router)
